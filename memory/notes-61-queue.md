...
✅ **Re-verified against current code 2026-09-06 (PM autonomous sweep) — see resolution column.**
`backlog-master-2026-08-19.md` had flagged this whole block "mostly unconfirmed as fixed"; this
pass checked all 14 individually rather than leaving that flag standing.

| # | Item | Notes | 2026-09-06 resolution |
|---|---|---|---|
| 1 | Store Tiles → Daypart Pace: rename **PM → Snack** | pure label | ✅ Already fixed — `store-analytics.js:1638`'s `DAYPARTS_DEF` already reads `label:'Snack'` (id stays `'pm'` internally, display label already renamed). |
| 2 | Forecast Table → **Biggest Miss** counts the current partial day | Same class of bug as the swing alarm counting a part-finished day as whole. Either exclude today, or — better — wire in the hourly Sales/GC projections we already have. **Same fix needed in Backtest Accuracy.** | ✅ Already fixed — v4.917/v4.924 excluded the current business day from both Forecast Table and Backtest Accuracy district-wide (`store-dash.js:652-654`'s `_todayBiz`/`businessDate()` guard). Confirmed live in code, not just the changelog claim. |
| 3 | Period forecast (bottom): last complete day has **no labor** at 10:00 am | Populate from what is available, update as finals land | ❓ Unconfirmed either way — no specific code location found matching "Period forecast (bottom)"; reads like a point-in-time data-freshness observation from the moment of the 08-08 review (labor finals hadn't posted yet at 10am that day) rather than a reproducible structural bug. Left open rather than guessed at. |
| 4 | Scorecard: **missing 2-WK averages** | | ✅ Already fixed — `OpsScorecard` (`store-dash.js:1023`) renders a `2-Wk` column (`q2`) as the primary column in both the Operational Metrics and Labor & Scheduling tables. |
| 5 | Intelligence Brief: **all pastel, including the text** — unreadable | contrast pass | ✅ Already fixed — dedicated PR (`cc789f5`, "Fix unreadable Intelligence Brief text in light mode"), covered by `src/__tests__/intelligence-brief-light-mode-contrast.test.js` (6/6 passing). |
| 6 | Action Plan: *"I like it… anything new to enhance?"* | open question, not a bug | Not a bug — no action needed unless the owner has new specifics. |
| 7 | Shift Analysis: **TPPH not populating** in several places; **missing Parked at Dinner** | TPPH is now a derived metric — check it is sourced through `metric-source.js` | ✅ Fixed 2026-09-06 — DOW summary tables already sourced TPPH through `metricSeries()`/`metric-source.js` (confirmed, `store-analytics.js:112`). But `PeaksTab`'s (aka the "3 Peaks"/Shift Analysis peak-daypart cards, `store-dash.js`) per-daypart KPI grid computed `p.tpph` (`analyzePeaks`, line 190) and used it in the AI prompt, but never rendered it as a visible tile — genuinely missing from the UI, not a sourcing gap. Added a TPPH tile to the breakfast/lunch/dinner card grid (`store-dash.js:1373-1379`). Parked at Dinner was already present (see #8). |
| 8 | 3-Peaks: **missing Parked at Dinner** | | ✅ Already fixed — `PeaksTab`'s per-daypart card grid (`store-dash.js:1361-1387`) maps over `['breakfast','lunch','dinner']` and renders a "DT Parked %" tile for every daypart including Dinner. No dinner-specific gap found. |
| 9 | Register Audit: **Refunds total not rounded to 2 dp** | | ✅ Already fixed — `register-audit.js:61-70`'s own comment documents the exact fix: `refundCash`/`refundCashless`/`manualRef` are all `Math.round(...*100)/100`, and the dollar/count confusion that caused the original bug (`refundCnt` briefly summing a dollar column) is resolved with `refundCnt` now a pure count. |
| 10 | Register Audit: **drill into each employee** — "since we can" | | Still open — feature-sized (per-employee drill-down UI), not attempted unattended. |
| 11 | Register Audit: **surface ALL audit metrics we already have.** For any without a %, compute `event $ ÷ total drawer sales` | biggest item in this block | Still open — feature-sized (audit which metrics exist vs. surfaced, design the %-of-sales computation), not attempted unattended. |
| 12 | **Tishomingo (43380)** shows "new model store", forecast model health null — open >1 yr, should be fixable. | | ✅ Fixed 2026-09-06 — root cause: `recentOnly` in `DEFAULT_MODEL_ASSIGNMENTS` conflates a genuinely new/ramp-up location (Ponce de Leon, 43701, monthly/yearly `n:0`) with a long-open store where DI specifically isn't viable despite real AE/DOW/LY calibration (Elgin 33222, Mossy Head 37566, Tishomingo 43380 — all real `n` in the hundreds at monthly/yearly horizons). `modelHealthScore`/`computeModelHealth` (`engine/forecast.js`) labeled all four "New Store" alike. Now branches on whether real monthly/yearly calibration exists: established-but-DI-unviable stores get a new "DI N/A" label and a statement that doesn't claim they're new; genuinely new stores are unaffected. No numeric score is fabricated either way — still `null`/N/A, only the label/statement text changed. Covered by `src/__tests__/model-health-recentonly-mislabel.test.js`. |
| 13 | **Records** should be all-time, not limited to the date selection. Wants top 3 per category + most recent record broken + near misses | | ❌ NOT built, and not the bug described — `ds.records[loc]` (manual-upload "Records" file, `parseRecords()`) already renders as "Store All-Time Records" and is NOT date-scoped (confirmed in code). But it only holds one current record-holder value+date per category from a manually uploaded spreadsheet — no top-3, no near-miss detection, no auto-computed "record broken" alert from live daily data. The real ask is a genuinely unbuilt, product-judgment-heavy feature (which metrics count as records? auto-computed from which data source?) — left unscoped rather than guessed at. |
| 14 | District Overview: make the **Critical chip clickable** → list all critical items. Same for **Watch flags** | | ✅ Already fixed — `analytics.js:5484`'s own comment cites this exact item ("Notes 61 DV14"): the "⚠ Needs Attention" panel has clickable `Critical`/`Watch`/`Acknowledged` severity chips (`sevChip(...)`) that filter the findings list below. |
