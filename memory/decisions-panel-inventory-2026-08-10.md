...
| 8 | `AnomalyPanel` | **✅ DONE (2026-08-10, issue #127).** Event-tagged baseline exclusion — drops closure/remodel/weather days from the DOW baseline so a remodel doesn't poison the mean. `runScan`'s trimmed mean blunts outliers but doesn't know *why* a day was odd; a synthetic 2-week-disruption test confirms trimming alone doesn't fully catch it (baseline moved ~6% once excluded outright). Ported into `runScan` faithfully, including a finding worth flagging: "closure" and "remodel" aren't real `EVENT_TYPES` keys anywhere in this codebase — only "weather" is — so this exclusion has only ever functioned for weather-tagged days, in the orphan and now here too; not silently "fixed" by guessing the author's intent. Also measured against production: 2,651 `org_events` rows, zero tagged weather/closure/remodel — today this is a no-op live, ready for the next tag. The orphan panel is deleted outright (its render had drifted from its own engine's field names — every row would render blank even if reachable). | `runScan` in `analytics.js` (already owns the event registry + tagging UI) |
