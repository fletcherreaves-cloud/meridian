...
|---|---|---|---|
| 1 | `DevDashboard` (orphan) | **✅ DONE (2026-08-10, issue #123).** Data Audit coverage grid — per-store × per-source row counts, first/last date, coverage %, Full/Partial pill. No live equivalent. **Would have surfaced the `labor_rows` staleness at a glance instead of two weeks late.** ~30 lines, relocated verbatim (same math, same 90/70 thresholds) as `DataManagerPanel`'s new **Coverage** tab (`analytics.js`, `auditGrid` useMemo). The Settings Dump (curated forecast-config JSON) moved into the LIVE `DevDashboard` (`management.js:27`) as a new "Forecast Config (Quick Glance)" card — minus the `storesLoaded` count, which would have required threading `ds` through `Settings`→`DevDashboard` for one field; not worth it for a dispensable part of the dump. Engine Trace tab dropped outright — `ForecastAudit` (standalone since #114) supersedes it, including fixing the same `k`/`v`-as-siblings rendering bug the orphan's version had. The orphan itself, its lazy import, `showDev` state, and its `ORPHANS` entry are all deleted — resolves the two-components-named-`DevDashboard` collision permanently. | `DataManagerPanel` as a Coverage tab |
...
| 9 | `AIInsightsLog` | **✅ DONE (2026-08-10, issue #128).** Category taxonomy (ops/ctrl/labor/sales/weather/anomaly/other + colors) — well-chosen buckets, exist nowhere else — now lives on `TaskQueuePanel` as an optional category facet (badge, picker on create + on an existing task, filter pills that only render once something is actually categorized) plus an optional store scope (`tasks.category`/`tasks.loc`, live in Supabase). Checked with the owner whether the real localStorage journal (`mf_insights`, device-local, not inspectable from this sandbox) held anything worth migrating first — empty, nothing to carry forward. `AIInsightsLog`, its localStorage helpers, and its dead lazy import/state/render wiring in `App.js` are all deleted. The unfinished idea (every record carried `source:'manual'`, nothing ever wrote a non-manual one) was NOT finished here as originally planned: a first pass had `AIBacktestScanner`'s `runScan` auto-file a title-deduped Task Queue entry per store per scan, but the owner settled the real design for that in **issue #134 — a standalone Insight Ledger panel, situation-key dedup, behind a measure-first gate** — a different (and better) shape than a scanner-side auto-filer. The auto-file block was cut back out rather than ship a second, competing implementation; that capability belongs to the Ledger's own build. | taxonomy → `TaskQueuePanel` as a category facet; auto-file → deferred to the Insight Ledger (issue #134), not built here |
| 8 | `AnomalyPanel` | **✅ DONE (2026-08-10, issue #127).** Event-tagged baseline exclusion — drops weather-tagged days from the DOW baseline so a multi-week disruption doesn't poison the mean. `runScan`'s trimmed mean blunts outliers but doesn't know *why* a day was odd; a synthetic 2-week-disruption test confirms trimming alone doesn't fully catch it (baseline moved ~6% once excluded outright). Ported into `runScan`, with two corrections found on PM review: "closure" and "remodel" aren't real `EVENT_TYPES` keys anywhere in this codebase — only "weather" is — so those two dead strings were removed from the live condition rather than left in pretending to check cases that can't occur; and tag-driven exclusion is a deliberate, bounded exception to the v4.924 "measured, not tagged" principle (which replaced this exact pattern elsewhere after it silently zeroed calibration for a heavily-tagged store) — safe here because the `vals.length<4` guard fails safe (no baseline) rather than wrong, and now `console.warn`s by store+DOW if event tags ever empty a bucket, instead of going silently blind on that day. Also measured against production: 2,651 `org_events` rows, zero tagged weather — today this is a no-op live, ready for the next tag. The orphan panel is deleted outright (its render had drifted from its own engine's field names — every row would render blank even if reachable). | `runScan` in `analytics.js` (already owns the event registry + tagging UI) |
