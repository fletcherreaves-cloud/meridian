...
| 9 | `AIInsightsLog` | **✅ DONE (2026-08-10, issue #128).** Category taxonomy (ops/ctrl/labor/sales/weather/anomaly/other + colors) — well-chosen buckets, exist nowhere else — now lives on `TaskQueuePanel` as an optional category facet (badge, picker on create + on an existing task, filter pills that only render once something is actually categorized) plus an optional store scope (`tasks.category`/`tasks.loc`, additive migration `supabase/schema-tasks-category.sql`). The unfinished idea (every record carried `source:'manual'`, nothing ever wrote a non-manual one) is now finished: `AIBacktestScanner`'s `runScan` auto-files a Task Queue entry, category Anomaly, for the single worst finding per store per scan, title-deduped against existing open tasks so re-running the scanner doesn't pile up duplicates. Checked with the owner whether the real localStorage journal (`mf_insights`, device-local, not inspectable from this sandbox) held anything worth migrating first — empty, nothing to carry forward. `AIInsightsLog`, its localStorage helpers, and its dead lazy import/state/render wiring in `App.js` are all deleted. | taxonomy → `TaskQueuePanel` as a category facet; auto-file → `saveTask` from `AIBacktestScanner` |
