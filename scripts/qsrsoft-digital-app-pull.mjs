#!/usr/bin/env node
// scripts/qsrsoft-digital-app-pull.mjs
// QSRSoft Digital App → per-store monthly Digital App GC/R/D (review metric).
// Feeds the Performance-Review Digital App metric → digital_app_monthly.
//
// Endpoint (confirmed 2026-07-28, the mobileApp report; one call = all stores):
//   GET https://api.reports.myqsrsoft.com/reporting/v2/sales/qtr-hr-sales
//       ?catalogType=sales&timeSegment=openClose&segmentBy=summary&segmentNames=open-close
//       &segmentsSelected=open-close&nsd=d&nsn=<csv>&orgId=<org>&enterpriseName=McDonalds
//       &startDate=YYYY-MM-01&endDate=YYYY-MM-DD&dsd=s&compType=trading
//       &daysOfWeek=1,2,3,4,5,6,7&weekStart=3&timeInterval=summary&selectCols=<components>
//   → { result: [ {storeNum, allNetSales, numberOfDays, qualifiedDay, …components…} ] }
// "Digital App" is COMPOSED (QSRSoft does not return a rollup column): Mobile Order
// Ahead + Scanned Offers (all 3 variants) + Loyalty Self-ID Earn + Internal/GMA
// Delivery — reconciled exactly (sales AND GCs) to the owner's export across stores.
// parseDigitalAppApi() in src/engine/people-reports.js does the composition; GC/R/D =
// Digital App GCs ÷ qualified rest-days (the window's qualifiedDay).
//
...
// Optional: DIGITAL_PERIOD=YYYY-MM · ROSTER_STORES=3708,… · QSRSOFT_DEBUG=1

import { createClient } from '@supabase/supabase-js';
import { parseDigitalAppApi } from '../src/engine/people-reports.js';

const API_BASE   = 'https://api.reports.myqsrsoft.com';
const ORG_ID     = 'a546d4ef-684a-4f25-8bc0-6580af068875';
const ENTERPRISE = 'McDonalds';
const REPORT_URL = 'https://v3.myqsrsoft.com/reports/mcd/digital/mobileApp';
const DEBUG      = process.env.QSRSOFT_DEBUG === '1';

// The component fields parseDigitalAppApi composes, + total sales for % of sales.
const SELECT_COLS = [
  'transactions', 'allNetSales',
  'mobileOrderAheadAllNetSales', 'scannedOfferOnlyRewardsAllNetSales', 'scannedOfferNoRewardsAllNetSales',
  'scannedOfferRewardsAndOfferAllNetSales', 'loyaltySelfIDEarnAllNetSales', 'intDeliveryAllNetSales',
  'mobileOrderAheadTransactions', 'scannedOfferOnlyRewardsTransactions', 'scannedOfferNoRewardsTransactions',
  'scannedOfferRewardsAndOfferTransactions', 'loyaltySelfIDEarnTransactions', 'intDeliveryTransactions',
].join(',');
...
function currentPeriod() {
  if (process.env.DIGITAL_PERIOD) return process.env.DIGITAL_PERIOD;
  const d = new Date();
...
function buildUrl(period) {
  ...
  const params = new URLSearchParams({
    catalogType: 'sales', timeSegment: 'openClose', segmentBy: 'summary',
    segmentNames: 'open-close', segmentsSelected: 'open-close', nsd: 'd', nsn: STORE_NSNS.join(','),
    orgId: ORG_ID, enterpriseName: ENTERPRISE, startDate: first, endDate: last, dsd: 's',
    compType: 'trading', daysOfWeek: '1,2,3,4,5,6,7', weekStart: '3', timeInterval: 'summary',
    selectCols: SELECT_COLS,
  });
  return `${API_BASE}/reporting/v2/sales/qtr-hr-sales?${params}`;
}
...
async function upsert(rows) {
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    const { error } = await supabase.from('digital_app_monthly').upsert(slice, { onConflict: 'loc,period_month' });
...
async function fetchDirect(token, period) {
  ...
  if (DEBUG) console.log(`[digital] GET ${url.slice(0, 140)}…`);
...
async function fetchViaPlaywright(period) {
  ...
  try {
    ...
    await snap('digital-01-landing.png');
    ...
    if (!token) { console.error('[auth] ✗ could not capture a reporting token'); await snap('digital-error.png'); return null; }
    console.log(`[auth] ✓ token captured (${token.length} chars) — fetching digital app…`);
    ...
    await snap('digital-final.png');
    if (result.error) { console.error('[digital] in-browser fetch error:', result.error); return null; }
    return extractRows(result.body);
  } catch (e) {
    console.error('[auth] Playwright error:', e.message);
    await snap('digital-error.png');
    return null;
  } finally {
    await browser.close();
  }
...
async function main() {
  ...
  console.log(`[digital] period ${period} × ${STORE_NSNS.length} stores`);
  ...
  if (rawRows == null) { console.error('[digital] ✗ no auth method succeeded'); process.exit(1); }

  const byLoc = parseDigitalAppApi(rawRows);
  const rows = toRows(byLoc, period);
  const found = Object.keys(byLoc).length;
  console.log(`[digital] ${found} stores composed`);
  if (found < STORE_NSNS.length) console.warn(`[digital] ⚠ only ${found}/${STORE_NSNS.length} stores in response`);
  const saved = await upsert(rows);
  console.log(`[digital] ✓ ${saved} store rows upserted to digital_app_monthly for ${period}`);
  if (!saved) process.exit(1);
}

main().catch(err => { console.error('[digital] FATAL:', err); process.exit(1); });
