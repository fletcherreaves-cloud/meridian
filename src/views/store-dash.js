...
function ActionPlanTab({store, ds, settings, dateRange}) {
  ...
  const printPlan = () => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Action Plan — ${store.name}</title>
...
// PERFORMANCE CALCULATOR — RETIRED from this file (dispatch #199, 2026-08-28): moved into
// Performance Review → Customize → Calculator, the same "harvest-then-remove" move dispatch
// #135 item 3 already did for Targets Editor. See src/views/performance-calculator.js
// (PerformanceCalculatorSection) for the ported logic + the scoring-divergence and RBAC notes.
