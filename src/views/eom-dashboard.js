...
import { ledgerScopeDiff, ledgerBaselineDiff, closeWindowStartFor, itemCloseWindowRecount, recountVerdictText } from '../engine/eom-ledger-baseline.js';
...
export function EOMDashboardPanel({ stores, ds, settings, onClose, initialMode, initialStore }) {
  ...
  return h(RoutePanelShell, {
    ...
    mode === 'snapshot'
      ? (loading || fobPending)
        ? div({ style: { padding: '40px', textAlign: 'center', color: 'var(--text3)' } }, 'Loading…')
        : h(EOMTeamSnapshotPanel, { rows, period, scopeLabel: scopeLabel() })
      : null,
    mode === 'recount'
      ? (loading || fobPending)
        ? div({ style: { padding: '40px', textAlign: 'center', color: 'var(--text3)' } }, 'Loading…')
                  ...
                  if (open) {
                    ...
                    rowEls.push(h('tr', { key: loc + '-d' }, h('td', { colSpan: 9, style: { padding: '2px 7px 10px 24px', background: 'var(--surf2)' } },
                      ...
                      : h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
                        h('thead', { key: 'ih' }, h('tr', null, [
                          th('Item', { left: true }), th('Class', { left: true }),
                          th('Session $'),
                          ...Array.from({ length: maxR }, (_, k) => th(`Recount ${k + 1}`)),
                          th('Final $'), th('Δ|var|'), th('Move'), th('Recounted')])),
                        h('tbody', { key: 'ib' }, its.map((it, j) => {
                          const recs = it.recounts || [];
