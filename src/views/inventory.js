...
function generateBulkInventoryReport(allInvRows, threshold, excldWrap, classKey, settings){
    ...
    return`<div style="page-break-before:always;padding:0 0 24px">
      <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:2px solid #f5bc00;padding-bottom:10px;margin-bottom:14px">
        <div><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.5px">Meridian · Inventory</div>
          ...
          <div style="font-size:9px;color:#64748b">Supervisor: ${whoRan(loc,new Date())||inv.sup||'—'} · Operator: ${inv.op||'—'} · Delivery: ${inv.del||'—'} · ${classKey} items</div></div>
  ...
  <table style="width:100%;border-collapse:collapse;font-size:9px"><thead><tr><th style="text-align:left;padding:4px 8px;border-bottom:.5px solid rgba(255,255,255,.1);color:#64748b">Location</th><th style="text-align:left;padding:4px 8px;border-bottom:.5px solid rgba(255,255,255,.1);color:#64748b">Supervisor</th><th style="text-align:right;padding:4px 8px;border-bottom:.5px solid rgba(255,255,255,.1);color:#64748b">Overstock Items</th><th style="text-align:right;padding:4px 8px;border-bottom:.5px solid rgba(255,255,255,.1);color:#64748b">Excess Value</th></tr></thead><tbody>${byLoc.map((l,i)=>`<tr style="${i%2?'background:rgba(255,255,255,.02)':''}"><td style="padding:4px 8px;font-weight:800;font-size:11px;color:#f5bc00">${l.name}</td><td style="padding:4px 8px;color:#94a3b8">${whoRan(l.loc,new Date())||(INV_ORG_COORDS[l.loc]||{}).sup||'—'}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;color:#f97316">${computeInvSections(classRows(l.loc),threshold,excldWrap,true).overstk.length}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;font-weight:700;color:#ef4444">$${l.val.toFixed(2)}</td></tr>`).join('')}</tbody>
