...
// reached by both a static and a dynamic import path lands in the eager entry chunk regardless
// of which export is used — see changelog-data.js's header for the full reasoning.
...
// #230: the About modal's changelog list — the only consumer of the ~98 KB-gzipped
// MERIDIAN_CHANGELOG array (changelog-data.js), lazy for the same reason every other
// rarely-opened panel here is.
...
// #230 (2026-08-12): the full MERIDIAN_CHANGELOG array (366 entries, ~98 KB gzipped in
// isolation) moved to changelog-data.js so it stops shipping in the entry chunk — the About
// modal (changelog-panel.js, lazy) is its only real consumer. LATEST_CHANGELOG_ENTRY is the
// one piece App.js still needs synchronously (footer text, window.__MERIDIAN_VERSION__), kept
// as a named export from a SEPARATE, standalone module (changelog-latest.js) — never from
// changelog-data.js itself, since a module reached by both a static and a dynamic import path
// gets pulled into the eager entry chunk regardless of which export is used.
