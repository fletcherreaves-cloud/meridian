...
const MERIDIAN_CHANGELOG  = [
  {version:'4.996', date:'2026-08-12', changes:[
    'Issue #211 (Push 4, "the generic shell") — first slice, plus the scoping pass that precedes the shell itself. #211 is explicitly the largest structural job on the board (memory/project-inventory-control-redesign.md: consolidate 6 inventory/food-cost panels into one scope×period×class shell modeled on above-store-onepager.js, then instantiate a second time for Labor) and the issue itself says several design questions must be confirmed "before starting," not guessed at. Ran a full research pass against the live code (not the design doc\'s own 2026-08-11 reading) before writing any shell code: confirmed above-store-onepager.js\'s exact scope/period/section/drilldown/print architecture as the pattern to copy; found the design doc\'s "three report templates" claim needs a correction — Chronic Offenders is structurally a flat per-item table (template C\'s shape), not a per-store loop like its claimed siblings Count Reliability/Rubber-band, and EOM Supervisor needs a genuine 4th template (live editable cells with real Supabase persistence, DOM-based print instead of eom-dashboard\'s popup-window pattern, a Year/Month+grouping period unit, and a distinct "shaded boxes" rollup formula that explicitly excludes Total Food Cost/OT $ per a 2026-08-04 owner directive already in the code) rather than a preset of the district-KPI template; confirmed eom-share-view needs only a fixed single-store/single-period slice, never live scope/period controls, which resolves how the shell\'s planned LINK output mode should work; and confirmed the "identical fragmentation" claim for Labor concretely, not just by analogy — labor-tools.js\'s OperatorSummaryPanel and LaborAnalyticsPanel carry two independently-retyped copies of the same PERIODS array that have already silently drifted by one entry, and scheduling.js carries a third, independent Wednesday-anchored week vocabulary with two only-partially-shared implementations in the same file. Full findings appended to memory/project-inventory-control-redesign.md §12b (superseding §12\'s open-questions list) rather than filed as a separate doc, since #211\'s existing memory file is the one future sessions will read before touching this area. One open design question is NOT resolved by code-reading and needs the owner\'s call before Labor instantiation specifically (Food Cost is unaffected — its class axis was already fully decided): should Labor\'s crew-vs-management "class" dimension (now confirmed real and already in the data — labor-basis.js\'s tCrewLabor resolver, eom-supervisor.js\'s crew_labor_dollars field, distinct from "position," which does not exist anywhere in the data model) work as a toggle-able filter axis matching Food Cost\'s class dropdown, or as a side-by-side column split the way eom-supervisor.js already shows it today? Asked, not answered yet — proceeding on the filter-axis default (matches the shell\'s own generic-by-design goal) until told otherwise, documented as a default, not an owner-confirmed decision. Shipped as this commit\'s actual code change, since it\'s low-risk and independently valuable regardless of how the rest of the shell design lands: the design doc\'s "free perf win" — eom-dashboard.js:1216-1243 independently re-fetched the FULL unscoped ~13,190-row qsr_fob table via loadQsrFob() on every period change, duplicating the copy App.js already loads once at startup into ds.qsrFobRows (the same pattern above-store-onepager.js already uses). Confirmed still present and untouched by #214/#218 (those touched Inventory Intelligence/qsr_inventory_summary, a different table) via git log before fixing, not assumed. Fixed by reading ds.qsrFobRows directly instead of re-fetching — both are the exact same full unscoped table (loadQsrFob() itself takes no period argument), so this is a pure duplicate-fetch removal, not a narrower query. The rest of #211 — the actual shell scaffold, report-template consolidation, and Labor instantiation — is real, substantial, unstarted work; this commit is scoping + one small, safe, independently-shippable piece of it, not the shell itself. 1322/1322 tests pass (no new tests — this slice removes a duplicate fetch with no new business logic; existing eom-dashboard tests, if any exist for this panel, would already cover fobRows\' downstream consumers). Entry chunk 820.18 KB -> 821.85 KB gzip (+1.67 KB — this changelog text, by far the longest entry to date given the scoping-pass detail; the actual code change is a net deletion and eom-dashboard.js itself is lazy, its own chunk shrank slightly, 219.17 KB -> 219.14 KB raw), 28.15 KB headroom remaining.',
  ]},
