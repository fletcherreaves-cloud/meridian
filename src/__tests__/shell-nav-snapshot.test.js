...
// Re-captured again 2026-09-05 for the new Trend Explorer panel (owner-requested, memory/
// project-trends-panel.md): a brand-new kind:'nav', section:'analytics', route:true, perm:
...
// Re-captured again 2026-09-19 (backlog survey candidate #2, "Save/Restore Session -- relocate
// in nav"): 'Save Session'/'Restore Session' dropped from the bottom of this list entirely.
// They were two bare navItem() calls tacked on after the SECTIONS/Admin render loop with no
// panel-registry.js id/section/perm -- exactly the dangling end-of-list pattern the section:
// rule exists to prevent, and 'Save Session' duplicated a home it already had (ProfileMenu's
// account-icon dropdown, 'Save session to file'). Both actions now live ONLY in ProfileMenu
// (shell.js) -- 'Restore session from file' added there as a new item right after the existing
