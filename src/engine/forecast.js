...
const KB_ARTICLES = {
  ...
  // PANEL INDEX — live map of every panel (mirrors memory/panel-catalog.md)
  panel_index: {
    title: 'Panel Index — every panel, explained',
    category: 'App Guide',
    summary: 'A live map of every Meridian panel, grouped like the sidebar — find where a workflow lives or see what each panel does at a glance.',
    detail: '**A live index of every Meridian panel, grouped the way the sidebar is. Use it to find where a workflow lives or to see what a panel does at a glance.**\n\n' +
      '**DAILY**\n- **Home** — command center + KPI overview.\n- **Needs Attention** — auto-flagged stores/metrics that need action.\n- **Daily Brief** — the morning KPI summary.\n- **Date-Range Report** — ad-hoc report over any dates.\n- **Events & Tags** — promo / known-event calendar that feeds forecasts & Smart Targets.\n\n' +
      '**PERFORMANCE**\n- **Org Summary** — org/district rollup scorecard.\n- **Rankings** — rank stores (or rolled-up Patch / Operator / State groups) across any KPI.\n- **Planning** — one panel, five tabs: **Targets** (monthly targets, saved to cloud) · **Monthly** (per-store monthly projection + patch rollups) · **Pace** (MTD actual vs target + run-rate) · **Yearly** (annual projection view) · **Smart Targets** (learned, defensible target per store for sales / labor % / FOB / speed, with backtest). Tabs load on demand.\n\n\n' +
      '**LABOR & SCHEDULING**\n- **Scheduling** — one hub, five tabs: **Labor Analytics** (TPPH, labor %, OT, Actual-vs-Need) · **Scheduling** (scheduling intelligence) · **Schedule Summary** (LifeLenz weekly band across all stores + per-station hours & cost) · **Labor Analysis** (weekly Fixed/Floor/VLH plan from the LifeLenz sync) · **Skills** (per-employee cross-training matrix). Tabs load on demand and follow your role.\n\n' +
      '**PEOPLE / HR**\n- **Performance Reviews** — weighted GM/manager reviews with scoring.\n- **Visit Readiness** — predicted PACE readiness score + risk drivers.\n- **Graded Visits** — PACE graded-visit data (CFV / RGR / EcoSure).\n\n' +
      '**OPERATIONS**\n- **Food Cost** — FOB / food-cost analysis.\n- **End of Month** — per-store EOM inventory troubleshooter.\n- **EOM Supervisor** — supervisor EOM rollup.\n- **Guest Voice** — SMG VOICE CSAT (comments + FullScale scorecard).\n- **3PO Delivery** — third-party / delivery channel mix.\n- **Promo / Discount ROI** — matched-day lift vs give-away; per-store verdicts.\n\n' +
      '**ANALYTICS**\n- **Signals** — live tracking-to-plan, anomalies, Speed of Service, Signal Lab + auto-correlation Scanner.\n- **DT Speed of Service** — drive-thru speed by station/daypart.\n- **SAGE** — Claude-powered advisor (RBAC-scoped, live data tools).\n- **Leadership One-Pager** — weekly cascade review (Owner→DO→Supervisor→GM) + the Weekly Business Review, which prints to PDF or downloads as an editable Word (.doc), blank or auto-filled, and re-shapes its content to the selected level.\n- **Forms Library** — the leadership review forms catalogued by cascade level, each with Print-blank and Word-blank actions.\n- **Feature Requests / Task Queue** — roadmap requests + data-issue tickets.\n- **Forecast Brief** — pre-forecast analysis of the upcoming period.\n- **Market Intelligence** — location + market/weather context.\n- **District View / Store One-Pager** — district rollup + single-store summary.\n\n' +
      '**TEST KITCHEN** *(experimental, dev-gated)*\n- A forecast/diagnostic lab: Projections, Proj vs Actuals, Forecast Models, DI Calibration, Forecast Accuracy, LifeLenz Gap/Bridge, plus AI tools (Anomaly Scan, Why Engine) and various store experiments. Being triaged — promote / merge / retire.\n\n' +
      '**ADMIN**\n- **Settings** — app settings.\n- **Panel Manager** — show/hide the optional/experimental sidebar panels (with a description of each).\n- **Changelog** — version history.\n- **Knowledge Base** — this reference.\n- **Metric Lineage** — a transparent registry of every metric that is calculated from other metrics: its exact formula, upstream inputs, and the source report/table, so any number can be verified against source.\n- **Data Manager** — uploads + on-demand Sync buttons.\n- **Save / Restore Session** · **Help**.',
  },
  ...
  lifelenz_bridge: {
    title: 'MBI vs LifeLenz Accuracy — WFM Comparison & Adjustment',
    category: 'App Guide',
...
function forecastDay(loc,date,ds,settings,casc,tgt,horizon,forceModel){
  ...
  if(_assignedModel==='ae'){
    ...
    if(_aeFcst&&_aeFcst>0){
      ...
      // #178 item 4: t2/t4/t6 used to be set to the raw dollar forecast (Math.round(_aeFcst))
      // instead of a YOY trend RATIO, which is what every other model path puts there and what
      // ForecastRow's "T2W/T6W — YOY Sales%" cells render as (r.t2*100).toFixed(2)+'%'. Since
      // all 27 stores are assigned model 'ae', this made every store's T2W/T6W column show a
