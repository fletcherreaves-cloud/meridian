...
| Visit Readiness | ✅ | **v4.501** — per-store PACE graded-visit readiness (CFV/RGRV/EcoSure) from daily ops metrics. Transparent weighted composite (Speed 35 / Accuracy 30 / Quality 20 / Leadership 15, each vs the store's own DEFAULT_TARGETS) + separate Food-Safety risk flag (waste/holding proxies) + per-store top risk drivers + last actual visit. Engine `src/engine/visit-readiness.js`. Standards + metric-mapping in `memory/project-graded-visits-pace.md`. Cleanliness = acknowledged data gap. **⚠️ EcoSure is NO LONGER a gap — a sample has landed.** `getThirdPartyFoodSafetyVisitReport&visitId=` on `propel.mcd.com` returns full per-question FS1..FS36 results, scores, cited reasons and `visitDate`; both full years are captured. So the real third-party food-safety audit is available to **replace** the waste/holding proxy, not merely calibrate it — and the proxy has already been measured wrong on a live store (it flagged elevated risk where the audit passed). ⚠️ This does NOT license the opposite claim that the proxy is worthless; the finding is explicit that nothing there justifies deleting it. Endpoint, payload shape and the ingest traps (trailing spaces in `questionCode`, `result` 0=pass/1=cited/-1=N/A, critical items scoring 0/0) are in `memory/finding-ecosure-propel-api-2026-08-22.md`. |
...
- **Touching a panel for any reason? Also check it against the panel contract (owner-stated
  ...
  radius**. Opportunistic, not a mandate to sweep all 93 panels (32 are `route:true` as of
  2026-09-01, re-measured by parsing the live `PANELS` array, not grepped — that ratio is expected
